...
ships as two published npm packages at version 1.5.0 with modular entrypoints,
1,248 automated tests across 100 files, a seven provider parity contract, and a
content addressed command line tool for reproduction and verification. Our
evaluation is assurance by construction: the guarantees are expressed as code level
invariants and property tests rather than as a latency benchmark. The result is a
small runtime that makes auditability and reproducibility a default property of
every multimodal model run.
\end{abstract}
...
version 1.5.0. The packages are live on the public npm registry. They are published
...
29,400 lines of production TypeScript across 130 non-test source files. The core
package has a deliberately small runtime dependency surface of three packages:
\code{canonicalize} for RFC 8785 encoding, \code{@standard-schema/spec} for schema
contracts, and \code{mime} for artifact typing. The Ed25519 reference oracle and the
schema library used in tests are development dependencies only, so a consumer of the
core package does not inherit them.

Version 1.5.0 adds modular public subpaths for \code{providers}, \code{audit},
...
and artifact preparation. The package still targets Node 24 for the full runtime,
while the audit, context, artifact, routing, tools, eval, and core adoption paths are
validated for Node 20 style modular consumers.
...
The same provider contract now exposes opt in native provider features for provider
only consumers. \code{ProviderRunRequest.nativeTools}, \code{nativeToolChoice}, and
...
reports structural differences. Together these commands turn a receipt from a passive
...
suite contains 1,248 automated test cases across 100 test files, with 1,086 in the
runtime package and 162 in the command line package, and it runs under a standard
TypeScript test runner. Figure~\ref{fig:tests} shows the distribution by package.
...
\begin{axis}[
  ...
  ylabel={test cases},
  ylabel near ticks,
  symbolic x coords={runtime package, CLI package},
  xtick=data,
  nodes near coords,
...
\addplot[draw=blue!60!black, fill=blue!30] coordinates {(runtime package,1086) (CLI package,162)};
\end{axis}
\end{tikzpicture}
\caption{Automated test cases by package. The suite totals 1,248 cases across 100
files, 1,086 in the runtime package and 162 in the command line package, and it encodes the
...
refresh, although capability negotiation can refine a profile at call time. The full
runtime package still targets Node 24, even though several modular subpaths are
validated for Node 20 style consumers. Finally, external audit helpers can sign and
replay evidence produced elsewhere, but they cannot prove that the external executor
used the claimed provider unless that executor supplies trustworthy raw envelopes.

% --- Future Work ----------------------------------------------------------
\section{Future Work}

Several extensions build naturally on the receipt primitive. External audit can become
...
332 model profiles, modular adoption paths, and 1,248 tests that encode the guarantees
as invariants. By making a signed and replayable receipt a default byproduct of every run, Lattice turns
