...
% IEEEtran prefers no extra vertical hacks. Keep the preamble small so the
% document compiles on a stock TeX Live or MiKTeX installation.

\newcommand{\code}[1]{\texttt{\small #1}}

\begin{document}

\title{Capability Receipts: Verifiable and Reproducible\\ Multimodal Model Orchestration}
...
Signatures use the Edwards-Curve Digital Signature Algorithm \cite{rfc8032}, the same
EdDSA construction widely deployed for compact and fast signatures.
...
Step & Operation \\
\midrule
1 & assemble raw receipt body \\
2 & redact body (record \code{redactions[]}, policy id) \\
3 & canonicalize redacted body (RFC 8785 JCS) \\
4 & base64 encode canonical bytes as payload \\
5 & build DSSE pre authentication encoding \\
6 & sign pre authentication encoding with Ed25519 \\
\bottomrule
\end{tabular}
...
\midrule
Deterministic routing & pure \code{routeDeterministically} & recorded decision, no I/O or randomness \\
