 ...
 * smuggled into tool `execute` bodies (D-02): budget derivation,
 * summary-return validation, classified failure routing, and receipt
 * minting all live HERE.
 *
 ...
 * failures (D-10) are never re-dispatched.
 *
 ...
 * rejects dispatch once `ancestry.length >= policy.maxDepth`.
 */
...
/** Structured tool-result error shape (D-09). */
interface CrewDispatchErrorBody {
...
export function createCrewDispatcher(
  ...
  async function dispatchToolUse(
    ...
    // (3) Build the child AgentIntent and run the existing loop.
    ...
    const childResult = await runAgentInternal(childIntent, ctx.config);
    ...
    if (childResult.kind !== "success") {
      return errorResult(classifyChildFailure(childSpec.id, childResult));
    }
 ...
 * `no-contract-match` for child cost-budget exhaustion) plus crew-ceiling
 * breaches. Recoverable (D-09): iteration/wall-time exhaustion and
 * STUCK_REASONS stalls — the parent MAY re-dispatch.
 */
function classifyChildFailure(childId: string, failure: AgentFailure): CrewDispatchErrorBody {
  return {
    kind: failure.kind,
    reason:
...
function errorResult(body: CrewDispatchErrorBody): { readonly content: string } {
