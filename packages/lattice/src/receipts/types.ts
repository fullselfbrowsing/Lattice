// Receipt protocol type spine. These wire shapes are compatibility contracts:
//   - CapabilityReceiptBody is the signed payload schema.
//   - ReceiptEnvelope uses DSSE with a fixed payloadType.
//   - ReceiptSigner keeps raw private keys outside the runtime.
//   - KeySet / KeyEntry / KeyState define key rotation.
//   - VerifyResult / VerifyError form a typed failure union.
...
interface CapabilityReceiptBodyFields {
  ...
  // v1.2 model-class tag. Optional for legacy v1.1 receipts and
  // synthetic/unknown routes; populated from the strict registry when
  // runtime issuance has a known selected provider/model.
  readonly modelClass?: TrainingClass;
  // v1.3 receipt-chain link — additive and optional, with no schema
  ...
  // v1.3 provenance root. Commits to descriptor-only artifact
  ...
  // v1.1 step-marker fields. All optional; populated by callers when
