...
must_haves:
  truths:
    - "Schema files use JSON Schema draft 2020-12 with additionalProperties:false, flat per-version files (D-06)"
    - "I-JSON constraints are encoded via maximum:9007199254740991 (safe-int) and the costUsd decimal-string pattern (D-07)"
    - "Each schema carries $comment backstop text mirroring the normative-prose rules JSON Schema cannot enforce (D-08)"
