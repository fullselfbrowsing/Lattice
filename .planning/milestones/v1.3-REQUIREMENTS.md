# Requirements Archive: v1.3 Public Release + Model-Aware SDK + Multi-Agent Surface
...
> **⊘ SUPERSEDED (2026-06-15).** The five canary categories below — `CAN-*`, `UNIT-*`, `INTEG-*`, `COST-*`, `DISPATCH-*` (23 REQ-IDs, Phases 30–32) — were descoped at v1.3 close. The dedicated synthetic `fullselfbrowsing/lattice-canary` repo is replaced by integrating FSB (the real downstream product) as an `@full-self-browsing/lattice` npm consumer, which dogfoods the published tarball directly. See `milestones/v1.3-MILESTONE-AUDIT.md` for rationale. These remain unchecked **by design, not as gaps.**
...
**87 authored REQ-IDs** across **20 categories** are mapped in this file. **64 / 87 shipped** as of the 2026-06-11 Phase 29 stable publish closure. The remaining **23 (canary: `CAN` / `UNIT` / `INTEG` / `COST` / `DISPATCH`, Phases 30–32) were SUPERSEDED at v1.3 close on 2026-06-15** — descoped in favor of FSB-via-npm dogfooding (see `milestones/v1.3-MILESTONE-AUDIT.md`), not left incomplete. Accounting: 64 shipped + 23 superseded = 87/87 resolved.
