...
| CLI-02       | 11    | packages/lattice-cli/test/showcase-e2e.test.ts case 3                          | `lattice repro <receipt-path>` loads the receipt, verifies the signature, materializes the envelope from `.lattice/fixtures/<sha256>.bin`, applies the sidecar's `{ task, outputs, policy, contract, rawOutputs }`, calls `replayOffline`, and diffs the recomputed outputHash against the receipt. Case 3 (Phase 13.1) HARD-asserts exit 0 with `verdict=match` via the sidecar. |
...
| EVAL-02      | 12    | packages/lattice-cli/test/showcase-e2e.test.ts cases 5 + 6                     | Case 5 confirms baseline-relative gating exits 0 on a clean run. Case 6 (Phase 13.1 closes the v1.1 boundary) HARD-asserts exit 1 + `regressionKind="cost-regression"` against a mutated baseline once the success fixture is sidecar-paired and replays cleanly. |
...
6. **Forward-compat hooks closed by Phase 13.1.** The two `replay-failed`-boundary forward-compat assertions in case 3 and case 6 have been flipped to HARD assertions in Phase 13.1 after the sidecar surface landed. Case 3 asserts exit 0 + `verdict=match`; case 6 asserts exit 1 + `regressionKind="cost-regression"`. See `.planning/phases/13.1-receipt-sidecar-support/13.1-03-SUMMARY.md` for the closure trail.
...
1. **RESOLVED by Phase 13.1.** `lattice repro verdict=match` was unreachable in v1.1 because the receipt envelope lacked the embedded outputs that `replayOffline` needs. Phase 13.1 adds a sidecar JSON file (`.lattice/sidecars/<id>.json` by convention, or `--sidecar` / `--sidecar-dir` flags) that carries `{ task, outputs, policy, contract }` plus an optional `rawOutputs` map into `materializeReplayEnvelope`. The success fixture now replays to `verdict=match` (see `packages/lattice-cli/test/showcase-e2e.test.ts` case 3, which is a HARD assertion in Phase 13.1 — exit 0 + `verdict=match`).

2. **RESOLVED by Phase 13.1.** Same root cause; the cost-regression gate is now reachable for fixtures with a sidecar. Case 6 of the e2e suite now asserts exit 1 + `regressionKind=cost-regression` against a mutated baseline as a HARD assertion (Phase 13.1).
