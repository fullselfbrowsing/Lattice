...
| **Full suite command** | `pnpm --filter @full-self-browsing/lattice typecheck && pnpm --filter @full-self-browsing/lattice test && python3 -m pytest clients/python/tests` |
| **Estimated runtime** | Under 120 seconds |

## Sampling Rate

- After every TypeScript task commit: run the receipt-focused Vitest command.
- After every Python task commit: run `python3 -m pytest clients/python/tests`.
...
| 57-02-01 | 02 | 2 | SIGBR-01, SIGBR-02, SIGBR-05 | T-57-04 | Python mint/build_pae are corrected-only | unit | `python3 -m pytest clients/python/tests/test_mint.py` | existing | pending |
| 57-02-02 | 02 | 2 | SIGBR-03, SIGBR-04, SIGBR-06 | T-57-01, T-57-02, T-57-03 | Python bridge mirrors TypeScript policy and diagnostics | unit | `python3 -m pytest clients/python/tests` | existing | pending |
| 57-02-03 | 02 | 2 | SIGBR-01..06 | T-57-01..T-57-04 | Both language suites pass together | integration | `pnpm --filter @full-self-browsing/lattice typecheck && pnpm --filter @full-self-browsing/lattice test && python3 -m pytest clients/python/tests` | existing | pending |
